...
function bukaFormTambahSiswa() {
    ...
    document.getElementById('inputSekolah').value = sessionStorage.getItem('tempSekolahDile') || '';
    nav('page-form-siswa');
}
...
function renderGridMateri() {
    ...
    dataMateri.forEach(m => { 
        ...
        <div class="card-materi" onclick="pilihMateri('${m.id_materi}')">
            <div class="icon-box">
            <img src="assets/${m.nama_materi}.png" alt="icon" />
            </div>
...
function renderAbsensi() {
    ...
    dataSiswa.forEach(siswa => {
        ...
        container.innerHTML += `<div class="content-box">
            <p style="font-weight:900; margin-top:0; color:#0288D1; font-size:16px;">${siswa.nama_siswa} <span style="font-size:13px; color:#9E9E9E;">(${status})</span></p>
...
function renderPenilaian() {
    ...
    dataSiswa.forEach(siswa => {
        ...
        container.innerHTML += `<div class="content-box">
            <p style="font-weight:900; margin-top:0; color:#0288D1; font-size:15px;">${siswa.nama_siswa} <span style="font-size:12px; color:#9E9E9E;">(${kehadiran})</span> ${statusText}</p>
...
function renderTabelEvaluasi() {
    ...
    dataSiswa.forEach((siswa, index) => {
        ...
        tbody.innerHTML += `<tr>
            <td>${index + 1}</td>
            <td style="text-align:left;">${siswa.nama_siswa}</td>
            <td style="font-weight:900; color:#0288D1;">${avgTotal}</td>
...
function bukaRincianEvaluasi(id_siswa) {
    ...
    dataMateri.forEach(materi => {
        ...
        htmlBintang += `<div class="star-row" onclick="bukaRiwayatMateri('${id_siswa}', '${materi.id_materi}')">
                <span style="width: 50%; text-align:left; color:#0288D1; font-weight:900;">➔ ${materi.nama_materi}</span>
    ...
    if(riwayat.length === 0) {
        ...
        riwayat.forEach(r => {
            ...
            tbody.innerHTML += `<tr style="color:${rowColor}; border-bottom:1px solid #E1F5FE;">
                ...
                <td style="font-weight:900; color:#0288D1;">${showNilai}</td>
